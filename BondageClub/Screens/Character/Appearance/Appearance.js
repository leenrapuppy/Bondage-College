	...
	if (GLVersion === "No WebGL" || !GLDrawCanvas || !GLDrawCanvas.GL || GLDrawCanvas.GL.isContextLost()) {
		...
		CommonDrawAppearanceBuild(C, {
			...
			drawImage: (src, x, y, alphaMasks, opacity, rotate) => {
				...
				DrawImageCanvas(src, C.Canvas.getContext("2d"), x, y, alphaMasks, opacity, rotate);
			},
			drawImageBlink: (src, x, y, alphaMasks, opacity, rotate) => {
				CommonDrawMarkDrawnAsset(C, src);
				DrawImageCanvas(src, C.CanvasBlink.getContext("2d"), x, y, alphaMasks, opacity, rotate);
			},
			drawImageColorize: (src, x, y, color, fullAlpha, alphaMasks, opacity, rotate) => {
				CommonDrawMarkDrawnAsset(C, src);
				DrawImageCanvasColorize(src, C.Canvas.getContext("2d"), x, y, 1, color, fullAlpha, alphaMasks, opacity, rotate);
			},
			drawImageColorizeBlink: (src, x, y, color, fullAlpha, alphaMasks, opacity, rotate) => {
				CommonDrawMarkDrawnAsset(C, src);
				DrawImageCanvasColorize(src, C.CanvasBlink.getContext("2d"), x, y, 1, color, fullAlpha, alphaMasks, opacity, rotate);
			},
			drawCanvas: (Img, x, y, alphaMasks) => {
				CommonDrawMarkDrawnAsset(C, Img);
				DrawCanvas(Img, C.Canvas.getContext("2d"), x, y, alphaMasks);
			},
			drawCanvasBlink: (Img, x, y, alphaMasks) => {
				CommonDrawMarkDrawnAsset(C, Img);
				DrawCanvas(Img, C.CanvasBlink.getContext("2d"), x, y, alphaMasks);
			},
		});
	} else {
